...
def transcribe_audio(audio_path, model_path):
    ...
    part_result = rec.FinalResult()
    results.append(part_result)

    words = []
    for r in results:
        if isinstance(r, str):
            r = eval(r)  # Convert string to dict
        if 'result' in r:
...
def process_video(input_path, output_path, model_path, font_path, font_size=200, y_offset=700):
    # Extract audio
    audio_path = f"{os.path.splitext(input_path)[0]}_temp.wav"
    ...
    if not word_timings:
        ...
        raise ValueError("No words were transcribed from the video")
